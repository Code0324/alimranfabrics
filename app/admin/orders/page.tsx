...
import { formatCurrency as formatPKR } from '@/lib/adminUtils';
import {
  CheckCircle,
  XCircle,
  Clock,
...
import type { AdminOrder as Order } from '@/lib/adminTypes';
...
function PaymentStatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium border ${styles[status] ?? 'bg-white text-[#6B6560] border-[#E0D8CC]'}`}>
      {status}
    </span>
  );
}
...
function OrderStatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium border capitalize ${styles[status] ?? 'bg-white text-[#6B6560] border-[#E0D8CC]'}`}>
      {status}
    </span>
  );
}
...
function MethodBadge({ method }: { method: string }) {
  const styles: Record<string, string> = {
    cod:       'bg-white text-[#6B6560] border-[#E0D8CC]',
    easypaisa: 'bg-green-950/60 text-green-400 border-green-800',
    jazzcash:  'bg-red-950/60 text-red-400 border-red-800',
  };
  return (
    <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium border ${styles[method] ?? 'bg-white text-[#6B6560] border-[#E0D8CC]'}`}>
...
function ScreenshotModal({ url, onClose }: { url: string; onClose: () => void }) {
  return (
    ...
    >
      <div
        className="relative bg-white border border-[#E0D8CC] rounded-xl overflow-hidden max-w-lg w-full"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between p-4 border-b border-[#E0D8CC]">
          <p className="text-sm font-medium text-[#1A1A1A]">Payment Screenshot</p>
          <button onClick={onClose} className="text-[#6B6560] hover:text-[#1A1A1A] transition">
            <X className="w-5 h-5" />
          </button>
        </div>
...
export default function AdminOrdersPage() {
  ...
  const handleVerifyPayment = async (orderId: string, decision: 'Paid' | 'Rejected') => {
    ...
    try {
      ...
      setOrders((prev) => prev.map((o) => (o.id === orderId ? (res as Order) : o)));
    } catch (err: any) {
            ...
            <button
              ...
              className={`flex items-center gap-1 px-2 py-1 rounded text-xs font-medium border disabled:opacity-50 transition ${
                action.next === 'cancelled'
                  ...
                  : 'bg-[#FAF7F2] text-[#B8963E] border-[#B8963E]/30 hover:bg-[#B8963E]/10'
              }`}
            >
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-serif font-bold text-[#1A1A1A]">Orders Management</h1>
          <p className="text-[#6B6560] text-sm mt-1">{total} total order{total !== 1 ? 's' : ''}</p>
        </div>
        <button
          onClick={fetchOrders}
          className="flex items-center gap-2 text-sm text-[#B8963E] hover:text-[#B8963E]/80 transition"
        >
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-full text-xs font-medium transition border ${
              paymentFilter === tab.value
                ? 'bg-[#B8963E] text-[#FAF7F2] border-[#B8963E]'
                : 'border-[#E0D8CC] text-[#6B6560] hover:border-[#B8963E]/50 hover:text-[#1A1A1A]'
            }`}
          >
            {tab.label}
          </button>
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-full text-xs font-medium transition border ${
              orderStatusFilter === tab.value
                ? 'bg-[#B8963E]/20 text-[#B8963E] border-[#B8963E]/50'
                : 'border-[#E0D8CC] text-[#6B6560] hover:border-[#B8963E]/30 hover:text-[#1A1A1A]'
            }`}
          >
      ...
      {loading ? (
        ...
        <div className="card text-center py-16">
          <Package className="w-12 h-12 text-[#6B6560] mx-auto mb-4" />
          <p className="text-[#6B6560]">No orders found for the selected filters.</p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-white text-left">
                <th className="px-4 py-3 text-xs font-semibold text-[#6B6560] uppercase tracking-wider">Order</th>
                <th className="px-4 py-3 text-xs font-semibold text-[#6B6560] uppercase tracking-wider">Customer</th>
                <th className="px-4 py-3 text-xs font-semibold text-[#6B6560] uppercase tracking-wider">Items / Total</th>
                <th className="px-4 py-3 text-xs font-semibold text-[#6B6560] uppercase tracking-wider">Payment</th>
                <th className="px-4 py-3 text-xs font-semibold text-[#6B6560] uppercase tracking-wider">Screenshot</th>
                <th className="px-4 py-3 text-xs font-semibold text-[#6B6560] uppercase tracking-wider">Order Status</th>
                <th className="px-4 py-3 text-xs font-semibold text-[#6B6560] uppercase tracking-wider">Actions</th>
              </tr>
            </thead>
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <p className="font-mono text-[#B8963E] font-bold text-xs">
                    ...
                    {/* Customer */}
                    <td className="px-4 py-3">
                      <p className="text-[#1A1A1A] font-medium">{order.customer_name}</p>
                      <p className="text-[#6B6560] text-xs mt-0.5">{order.customer_phone}</p>
                      <p className="text-[#6B6560] text-xs truncate max-w-[140px]">{order.customer_city}</p>
                    </td>

                    {/* Items / total */}
                    <td className="px-4 py-3">
                      <p className="text-[#1A1A1A] font-bold">{formatPKR(order.total_amount)}</p>
                      <p className="text-[#6B6560] text-xs mt-0.5">
                    ...
                    <td className="px-4 py-3">
                      {screenshotFullUrl ? (
                        <button
                          ...
                          className="flex items-center gap-1.5 text-xs text-[#B8963E] hover:text-[#B8963E]/70 transition border border-[#B8963E]/30 rounded px-2 py-1 hover:bg-[#B8963E]/10"
                        >
        ...
        <div className="flex items-center justify-between mt-6">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex items-center gap-1 px-3 py-1.5 rounded border border-[#E0D8CC] text-[#6B6560] text-xs hover:border-[#B8963E]/50 hover:text-[#1A1A1A] disabled:opacity-40 disabled:cursor-not-allowed transition"
            >
