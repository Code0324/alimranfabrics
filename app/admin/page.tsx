...
import { ordersAPI, productsAPI, usersAPI } from '@/lib/adminApi';
...
import type { AdminOrder, AdminProduct } from '@/lib/adminTypes';

interface SalesPoint { label: string; revenue: number; orders: number; }

function buildLast30Days(orders: AdminOrder[]): SalesPoint[] {
...
function buildStatusData(orders: AdminOrder[]) {
  const counts: Record<string, number> = {};
  orders.forEach((o) => { counts[o.status] = (counts[o.status] || 0) + 1; });
  return Object.entries(counts).map(([status, count]) => ({ status, count, color: '' }));
}

function buildCategoryData(products: AdminProduct[]) {
  const counts: Record<string, number> = {};
  ...
  const fetchData = async (silent = false) => {
    ...
    try {
      ...
      if (productsData.status === 'fulfilled') {
        setProducts(Array.isArray(productsData.value) ? productsData.value as AdminProduct[] : []);
      }
      if (usersRes.status === 'fulfilled') {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const users = usersRes.value.data as any;
        setCustomerCount(Array.isArray(users) ? users.filter((u: { role: string }) => u.role === 'customer').length : 0);
      }
    } catch (e) {
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button
          ...
          disabled={refreshing}
          className="self-start flex items-center gap-2 px-4 py-2 text-sm text-[#6B6560] border border-[#E0D8CC] rounded-lg hover:bg-[#F5F0E8] hover:text-[#1A1A1A] transition disabled:opacity-50"
        >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Total Revenue" value={formatCurrency(stats.totalRevenue)} icon={TrendingUp} iconColor="bg-[#B8963E]/10" iconTextColor="text-[#B8963E]" loading={loading} />
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 bg-white border border-[#E0D8CC] rounded-lg shadow-sm">
          <div className="flex items-center justify-between px-6 py-4 border-b border-[#E0D8CC]">
            <div>
              <h3 className="font-semibold text-[#1A1A1A]">Recent Orders</h3>
              <p className="text-[#6B6560] text-sm">Latest customer orders</p>
            </div>
            <Link href="/admin/orders" className="flex items-center gap-1.5 text-sm text-[#B8963E] hover:text-[#9A7A2E] transition">
            ...
            <div className="p-6 space-y-3">
              {Array.from({ length: 5 }).map((_, i) => (
                <div key={i} className="flex items-center gap-4">
                  <div className="h-4 bg-[#E0D8CC]/50 rounded animate-pulse w-20" />
                  <div className="h-4 bg-[#E0D8CC]/30 rounded animate-pulse flex-1" />
                  <div className="h-4 bg-[#E0D8CC]/50 rounded animate-pulse w-16" />
                </div>
              ))}
            </div>
          ) : recentOrders.length === 0 ? (
            <div className="p-12 text-center text-[#6B6560] text-sm">No orders yet</div>
          ) : (
            <div className="divide-y divide-[#E0D8CC]">
              {recentOrders.map((order) => (
                <div key={order.id} className="flex items-center gap-4 px-6 py-3 hover:bg-[#FAF7F2]/50 transition">
                  <div className="flex-shrink-0">{STATUS_ICON[order.status] ?? <Clock className="w-3.5 h-3.5 text-[#6B6560]" />}</div>
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-medium text-[#1A1A1A] truncate">{order.customer_name || 'Customer'}</p>
                  ...
                  <div className="flex-shrink-0 text-right">
                    <p className="text-sm font-semibold text-[#1A1A1A]">{formatCurrency(order.total_amount)}</p>
                    <p className="text-xs text-[#6B6560] capitalize">{order.payment_method}</p>
                  </div>
                </div>
              ))}
            </div>
