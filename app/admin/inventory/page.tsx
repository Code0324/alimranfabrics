...
import { formatCurrency, isLowStock, generateWhatsAppLink } from '@/lib/adminUtils';
import { useToast, useAdminStore } from '@/store/adminStore';
import {
...
export default function AdminInventoryPage() {
  ...
  const handleInlineEdit = async (product: Product) => {
    ...
    try {
      ...
      setProducts((prev) => prev.map((p) => (p.id === product.id ? (res as Product) : p)));
  ...
  const VIEWS: { label: string; value: FilterView; count: number; color: string }[] = [
    { label: 'All Products', value: 'all', count: stats.total, color: 'border-[#E0D8CC]' },
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="bg-white border border-[#E0D8CC] rounded-lg px-4 py-3">
          ...
          <p className="text-xs text-[#6B6560] mt-0.5">Total Products</p>
        </div>
        ...
        <div className="bg-white border border-[#E0D8CC] rounded-lg px-4 py-3">
          <p className="text-xl font-bold text-[#1A1A1A]">{formatCurrency(stats.totalValue)}</p>
          <p className="text-xs text-[#6B6560] mt-0.5">Inventory Value</p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-3 items-center">
        <div className="flex rounded-lg border border-[#E0D8CC] overflow-hidden">
          {VIEWS.map((v) => (
            <button
              key={v.value}
            ...
            >
              {v.label}
              <span className={`ml-2 text-xs font-bold ${filterView === v.value ? 'text-white/80' : 'text-[#6B6560]'}`}>
                ({loading ? '…' : v.count})
              </span>
            </button>
          ))}
        ...
        <select
          ...
          className="px-3 py-2 text-sm bg-white border border-[#E0D8CC] rounded-lg text-[#1A1A1A] focus:outline-none focus:border-[#B8963E]"
        >
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-[#F5F0E8] border-b border-[#E0D8CC]">
                <tr>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-[#6B6560] uppercase tracking-wider">Product</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-[#6B6560] uppercase tracking-wider hidden sm:table-cell">SKU / Category</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-[#6B6560] uppercase tracking-wider">Stock</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-[#6B6560] uppercase tracking-wider hidden md:table-cell">Price</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-[#6B6560] uppercase tracking-wider hidden lg:table-cell">Stock Value</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-[#6B6560] uppercase tracking-wider">Status</th>
              ...
              <tbody className="divide-y divide-[#E0D8CC]">
                {filtered.map((product) => {
                  ...
                  return (
                    <tr key={product.id} className={`hover:bg-[#F5F0E8]/50 transition ${isOut ? 'bg-red-50/30' : isLow ? 'bg-amber-50/30' : ''}`}>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          <div className="w-9 h-9 rounded-lg border border-[#E0D8CC] bg-[#F5F0E8] overflow-hidden flex-shrink-0 flex items-center justify-center">
                            {product.image_url ? (
                              ...
                              <Package className="w-4 h-4 text-[#6B6560]" />
                            )}
                          </div>
                          <p className="text-sm font-medium text-[#1A1A1A] truncate max-w-[160px]">{product.name}</p>
                        </div>
                      </td>
                      <td className="px-4 py-3 hidden sm:table-cell">
                        <p className="text-xs font-mono text-[#6B6560]">{product.sku}</p>
                        <p className="text-xs text-[#6B6560]">{product.category}</p>
                      </td>
                      <td className="px-4 py-3">
                        {isEditing ? (
                          <div className="flex items-center gap-1.5">
                            <input
                              ...
                              autoFocus
                              min="0"
                              className="w-20 px-2 py-1 text-sm border border-[#B8963E] rounded focus:outline-none focus:ring-1 focus:ring-[#B8963E]/30 bg-white"
                            />
                            <button
                          ...
                          <span className={`text-sm font-bold ${isOut ? 'text-red-600' : isLow ? 'text-amber-600' : 'text-[#1A1A1A]'}`}>
                            {product.stock}
                          </span>
                        )}
                      </td>
                      <td className="px-4 py-3 hidden md:table-cell">
                        <p className="text-sm text-[#1A1A1A]">{formatCurrency(product.price)}</p>
                      </td>
                      <td className="px-4 py-3 hidden lg:table-cell">
                        <p className="text-sm text-[#6B6560]">{formatCurrency(product.price * product.stock)}</p>
                      </td>
                      <td className="px-4 py-3">
                        {isOut ? (
