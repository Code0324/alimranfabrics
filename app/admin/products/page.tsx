...
import { formatCurrency, isLowStock } from '@/lib/adminUtils';
...
function FlagBadge({ label, icon: Icon, active }: { label: string; icon: React.ElementType; active: boolean }) {
  ...
  return (
    <span className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[10px] font-medium bg-[#B8963E]/10 text-[#B8963E] border border-[#B8963E]/20">
...
export default function AdminProductsPage() {
  ...
  const handleSave = async (data: Partial<Product>) => {
    if (editProduct) {
      ...
      setProducts((prev) => prev.map((p) => (p.id === editProduct.id ? (res as Product) : p)));
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            disabled={refreshing}
            className="p-2 text-[#6B6560] border border-[#E0D8CC] rounded-lg hover:bg-[#F5F0E8] hover:text-[#1A1A1A] transition disabled:opacity-50"
          >
            <RefreshCw className={`w-4 h-4 ${refreshing ? 'animate-spin' : ''}`} />
          </button>
          <button
            onClick={() => { setEditProduct(null); setModalOpen(true); }}
            className="flex items-center gap-2 px-4 py-2 text-sm font-semibold bg-[#B8963E] text-white rounded-lg hover:bg-[#9A7A2E] transition"
          >
      ...
      <div className="flex flex-wrap gap-3">
        ...
        {(search || filterCategory || filterBrand || filterActive !== 'all') && (
          <button
            ...
            className="px-3 py-2 text-xs text-[#6B6560] border border-[#E0D8CC] rounded-lg hover:bg-[#F5F0E8] hover:text-[#1A1A1A] transition"
          >
            Clear filters
          </button>
        )}
      ...
      {loading ? (
        ...
        <div className="bg-white border border-[#E0D8CC] rounded-xl p-16 text-center">
          ...
          <p className="text-[#6B6560] font-medium">No products found</p>
          <p className="text-[#6B6560] text-sm mt-1">Try adjusting your search or filters</p>
        </div>
      ) : (
            ...
            <table className="w-full text-sm">
              <thead className="bg-[#F5F0E8] border-b border-[#E0D8CC]">
                <tr>
                  ...
                  <th className="text-left px-4 py-3 text-xs font-semibold text-[#6B6560] uppercase tracking-wider hidden md:table-cell">Category / Brand</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-[#6B6560] uppercase tracking-wider">Price</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-[#6B6560] uppercase tracking-wider">Stock</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-[#6B6560] uppercase tracking-wider hidden lg:table-cell">Flags</th>
              ...
              <tbody className="divide-y divide-[#E0D8CC]">
                {filtered.map((product) => (
                  <tr key={product.id} className="hover:bg-[#F5F0E8]/50 transition">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-lg border border-[#E0D8CC] bg-[#F5F0E8] overflow-hidden flex-shrink-0">
                          {product.image_url ? (
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-xs text-[#6B6560] font-mono">{product.sku}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-4 py-3 hidden md:table-cell">
                      <p className="text-sm text-[#1A1A1A]">{product.category}</p>
                      <p className="text-xs text-[#6B6560]">{product.brand}</p>
                    </td>
                    <td className="px-4 py-3">
                      <p className="text-sm font-semibold text-[#1A1A1A]">{formatCurrency(product.price)}</p>
                      {product.compare_price && (
                        <p className="text-xs text-[#6B6560] line-through">{formatCurrency(product.compare_price)}</p>
                      )}
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-1">
                        ...
                        <button
                          ...
                          className="p-1.5 text-[#6B6560] hover:text-red-600 hover:bg-red-50 rounded-lg transition"
