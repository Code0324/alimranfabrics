...
import { generateWhatsAppLink } from '@/lib/adminUtils';
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      <div>
        ...
        <p className="text-sm text-[#6B6560] mt-0.5">Configure your store and admin preferences</p>
      </div>
      ...
      <Section icon={Package} title="Inventory Settings" subtitle="Configure low stock alerting thresholds">
        ...
        >
          <div className="flex items-center gap-3">
            ...
            />
            <span className="text-sm text-[#6B6560]">units</span>
          </div>
        </Field>
      </Section>
      ...
      {/* Actions */}
      <div className="flex items-center justify-between pt-2 border-t border-[#E0D8CC]">
        <button
          onClick={handleReset}
          className="flex items-center gap-2 px-4 py-2 text-sm text-[#6B6560] border border-[#E0D8CC] rounded-lg hover:bg-[#F5F0E8] hover:text-[#1A1A1A] transition"
        >
        ...
        <button
          onClick={handleSave}
          disabled={saving}
          className="flex items-center gap-2 px-6 py-2 text-sm font-semibold bg-[#B8963E] text-white rounded-lg hover:bg-[#9A7A2E] transition disabled:opacity-50"
        >
          {saving ? (
...
const inputCls =
  'w-full px-3 py-2 text-sm bg-[#FAF7F2] border border-[#E0D8CC] rounded-lg text-[#1A1A1A] placeholder-[#6B6560]/60 focus:outline-none focus:border-[#B8963E] focus:ring-1 focus:ring-[#B8963E]/20 transition';
...
function Section({
  ...
  return (
    <div className="bg-white border border-[#E0D8CC] rounded-xl overflow-hidden shadow-sm">
      <div className="flex items-center gap-3 px-6 py-4 border-b border-[#E0D8CC] bg-[#F5F0E8]">
        <div className="w-8 h-8 bg-[#B8963E]/10 rounded-lg flex items-center justify-center">
          <Icon className="w-4 h-4 text-[#B8963E]" />
        </div>
        <div>
...
function Field({
  ...
  return (
    <div>
      ...
      {children}
      {hint && <p className="text-xs text-[#6B6560]/70 mt-1">{hint}</p>}
    </div>
  );
}
  ...
  return (
    <label className="flex items-start gap-3 cursor-pointer group">
      <div
        ...
        className={`relative flex-shrink-0 w-10 h-6 rounded-full transition-colors duration-200 ${value ? 'bg-[#B8963E]' : 'bg-[#E0D8CC]'}`}
      >
        <span
      ...
      <span className="text-sm text-[#6B6560] group-hover:text-[#1A1A1A] transition leading-snug">{label}</span>
    </label>
  );
}
